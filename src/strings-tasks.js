...
function isString(value) {
  if (arguments.length === 0 || value == null) return false;
...
function sumOfCodes(str) {
  let sum = 0;
  if (arguments.length === 0 || typeof str !== 'string') return sum;
...
function formatTime(minutes, seconds) {
  const min = Math.abs(minutes).toString().padStart(2, '0');
  const sec = Math.abs(seconds).toString().padStart(2, '0');
  return `${min}:${sec}`;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < vowels.length; i += 1) {
    while (pos < str.length) {
      const foundPos = strToLowerCase.indexOf(vowels[i], pos);
      if (foundPos === -1) break;
      count += 1;
      pos = foundPos + 1;
    }
...
function isPalindrome(str) {
  ...
  const arrFilt = arr.filter(
    (el) => el !== ',' && el !== ' ' && el !== '?' && el !== '!' && el !== '.'
  );

  return arrFilt.join('') === arrFilt.reverse().join('');
}
...
function getCardId(value) {
  ...
  arrCards.forEach((card, ind) => {
    if (card === value) result = ind;
  });
